...
public class UserController {
    ...
    @GetMapping("/clients")
    public Page<ClientListDto> allClients(@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size, HttpServletRequest request) {
        requireLibrarian(request);
        return userService.getAllClients(page, size);
    }

    @GetMapping("/clients/by-membership/{membershipNumber}")
    public ClientListDto byMembership(@PathVariable String membershipNumber, HttpServletRequest request) {
        requireLibrarian(request);
        return userService.findClientByExactMembership(membershipNumber);
    }

    @PostMapping("/create")
    public String createUser(@Valid @RequestBody LibrarianCreateUserDto dto, HttpServletRequest request) {
        requireLibrarian(request);
        return authService.createUserByLibrarian(dto);
    }

    @PostMapping("/create-librarian")
    public String createLibrarian(@RequestBody LibrarianCreateUserDto dto, HttpServletRequest request) {
        requireLibrarian(request);
        return authService.createLibrarianByLibrarian(dto);
    }

    @PatchMapping("/{id}/phone")
    public ClientListDto updatePhone(@PathVariable Long id, @Valid @RequestBody UpdatePhoneDto dto, HttpServletRequest request) {
        requireLibrarian(request);
        return userService.updateUserPhone(id, dto.getPhoneNumber());
    }

    @GetMapping("/librarians")
    public Page<LibrarianListDto> allLibrarians(@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size, HttpServletRequest request) {
        requireLibrarian(request);
    ...
    public Page<LibrarianListDto> searchLibrarians(@RequestParam String q,
    ...
    public Page<ClientListDto> searchClients(@RequestParam String q, @RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size,
                                             HttpServletRequest request
    ) {
        requireLibrarian(request);
