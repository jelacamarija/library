...
@Component({
  ...
  <div class="mx-auto max-w-3xl px-4 py-6">
    ...
    <form class="bg-white border rounded-2xl p-6 space-y-4" (ngSubmit)="submit()">
      ...
      <div class="relative user-autocomplete">
        ...
        <div *ngIf="selectedUser()" class="text-xs text-gray-600 mt-1">
          ...
          <button type="button" class="ml-2 hover:underline" (click)="clearSelectedUser()">promijeni</button>
        </div>
      ...
      <div class="relative user-autocomplete">
        ...
        <div *ngIf="selectedBook()" class="text-xs text-gray-600 mt-1">
          ...
          <button type="button" class="ml-2 hover:underline" (click)="clearSelectedBook()">promijeni</button>
        </div>
...
private lastBookQuery = '';
 ...
 private fetchUsers(q: string): void {
  ...
  this.api.searchClientsByMembership(q, 0, 8).subscribe({
    next: (res: any) => {
      // ignorisi ako nije zadnji query
      if (this.lastUserQuery !== q) return;

    const list = (res?.content ?? []).map((u: any) => ({
  userId: u.userId ?? u.userID ?? u.id,   // ✅ dodaj u.userID
  name: u.name,
...
private fetchBooks(q: string): void {
  ...
  this.api.searchBooks(q, 0, 8).subscribe({
    next: (res: any) => {
      ...
      const list = (res?.content ?? []).map((b: any) => ({
  bookId: b.bookId ?? b.bookID ?? b.id,   // hvata bookID ako backend tako šalje
  title: b.title,
  author: b.author,
...
onDocumentClick(ev: MouseEvent): void {
  ...
  // ako klik nije unutar user bloka, zatvori user dropdown
  if (!target.closest('.user-autocomplete')) {
    this.userDropdownOpen.set(false);
  }

  // ako klik nije unutar book bloka, zatvori book dropdown
  if (!target.closest('.book-autocomplete')) {
    this.bookDropdownOpen.set(false);
  }
}
